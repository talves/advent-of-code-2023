...
fn process(input: &str) -> i32 {
    ...
    // dbg!(&parts);
    // dbg!(&symbols);
    let mut gears: HashMap<(usize, usize), i32> = HashMap::default();
    return parts
        .iter()
        .map(|part| {
            // dbg!(&gears);
            ...
            for x in part.start.checked_sub(1).unwrap_or(0)..=(part.start + part.len) {
                // dbg!(&x);
                if part.line > 0 {
                    if let Some(c) = symbols.get(&(part.line - 1, x)) {
                        if *c == '*' {
                            match gears.get(&(part.line - 1, x)) {
                                Some(val) => {
                                    value = *val * part.value;
                                    gears.remove(&(part.line - 1, x));
                                }
                                None => {
                                    gears.insert((part.line - 1, x), part.value);
                                }
                            }
                            break;
                        }
                ...
                if let Some(c) = symbols.get(&(part.line, x)) {
                    if *c == '*' {
                        match gears.get(&(part.line, x)) {
                            Some(val) => {
                                value = *val * part.value;
                                gears.remove(&(part.line, x));
                            }
                            None => {
                                gears.insert((part.line, x), part.value);
                            }
                        }
                        break;
                    }
